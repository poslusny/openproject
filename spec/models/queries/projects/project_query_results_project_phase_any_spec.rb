...
RSpec.describe ProjectQuery, "results of 'Project phase: Any' filter" do
  ...
  def self.remove_phase
    before do
      Project::Phase.destroy_all
    end
  end
  ...
  context "with a =d (on) operator" do
    before do
      instance.where("project_phase_any", "=d", values)
    end
  ...
  context "with a t (today) operator" do
    before do
      instance.where("project_phase_any", "t", [])
    end
  ...
  context "with a w (this week) operator" do
    before do
      instance.where("project_phase_any", "w", [])
    end
  ...
  context "with a !* (none) operator" do
    before do
      instance.where("project_phase_any", "!*", [])
    end
