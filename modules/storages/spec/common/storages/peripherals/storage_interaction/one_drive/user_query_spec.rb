...
RSpec.describe Storages::Peripherals::StorageInteraction::OneDrive::UserQuery, :webmock do
  let(:storage) do
  ...
  it "is registered" do
    expect(Storages::Peripherals::Registry.resolve("one_drive.queries.user")).to eq(described_class)
  end
  ...
  it ".call takes required parameters" do
    ...
    expect(method.parameters).to contain_exactly(%i[keyreq auth_strategy],
                                                 %i[keyreq storage])
  end
  ...
  it "responds with user details if request is successful", vcr: "one_drive/user_query_success" do
    ...
    expect(command_result.result).to eq(id: "a9023fd0-c421-4695-b83c-bb3ba67708d6")
  end
  ...
  it "responds with unauthorized if request is unauthorized", vcr: "one_drive/user_query_unauthorized" do
    ...
    expect(command_result.result).to eq(:unauthorized)
  end
end
